#!/usr/bin/env bash
# compile-macos.sh - compiles visual-regression-scanner into a standalone macOS
...
# and dist/visual-regression-scanner-vX.Y.Z-macos-<arch>.tar.gz ready to hand out.
#
...
#   xattr -dr com.apple.quarantine visual-regression-scanner
...
echo "Compiling visual-regression-scanner v$version ($arch) with Nuitka..."
rm -rf "$dist_dir"
started=$(date +%s)
...
tarball="$out_dir/visual-regression-scanner-v$version-macos-$arch.tar.gz"
rm -f "$tarball"
